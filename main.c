...
void setup_main_dirs()
{
	...
	char save_path[MAX_PATH], bram_path[MAX_PATH], config_path[MAX_PATH], images_path[MAX_PATH], syscards_path[MAX_PATH];
	...
	snprintf(syscards_path, MAX_PATH, "%s/%s", config.main_path, "syscards"); 
	...
	if(stat(syscards_path, &sb))
	{
		printf("syscards directory doesn't exist, creating\n");
		make_directory(syscards_path);
	}
}
