...
// Callee save: ebp, ebx, esi, and edi are callee save - only get 2 8bits
// here.

// These are the things needed, in estimated order of usage frequency:
// 8bit: a, x, y, p(*), sp(**)
// 16bit: pc
// 32bit state: memory base, cycles, memory table, stack/globals

// * Flags can be cached in x86 flags and they can also have redundant
// operations eliminated. The latter helps the former by sometimes allowing
// non-emulated instructions to damage flags if they know that

// ** sp would benefit from being in an 8/32bit register so long as PCE RAM is
// 256 byte aligned (not a very stringent requirement). Then the stack
// pointer can be incremented w/o wrapping around and it can be used directly
// for memory operations.

// pc will always be known at compile time so there's no reason to keep it
// in a register for x86.

// Anything that's a constant pointer doesn't need to be stored in a register;
// it can be kept in an immediate instead. So, save these for last and if a
// register is free go for it. That applies to all of the state except for
// the cycle counter. The immediate can always be combined with the
// reg_a + reg_b offsets and it can always be combined with another immediate
// as well.

// eax is a good temporary - you can use it as one 32bit or 2 8bit. But really
// a second temporary will be needed sometimes. That's pretty much unavoidable.

// There's a prerogative to keep x and y in their own registers, although this
// wastes an available register (and there weren't a lot to begin with). a
// doesn't have this limitation, but then there isn't a lot left that's good to
// pair with it. Probably good as a temporary. So:

// al: a
// ah: temporary (flags can go straight here)
// bl: x
// cl: y
// dl: sp
// esi: cycles
...
// ***: to be determined - memory base, memory table, stored p,
// temporary - p takes priority

// Code generation:
//

// Memory can be emulated like this:
// The 8 pages are either going to be static or dynamic - static things can
// be considered "locked." There is an assumption that there will in fact be
// static regions, but almost every game will always use fixed pages for I/O
// and fixed pages for work RAM - and these will usually be 0 and 1
// respectively. These can be treated as compile time constants. This doesn't
// mean that things break if the game does change them, just if we've used
// these constants we have to flush the translation cache. What we'll end up
// with are two address modes from the compiler's point of view: "simple" or
// "complex" (there's also immediate, which can probably be fit in the same
// enumerations)

// A simple address mode means that we can use a constant offset plus
// optionally an 8bit register (stored in a 32bit register) to determine it.
// This can often be folded into a computationally instruction, like how it
// was in the original Hu6280 code.

// A complex address mode means that the compiler has generated code to
// load the operand into a temporary, then code should be generated to use
// this temporary.

// For the direct page modes, we'll usually be able to tell which page it
// belongs to, and if it's static + RAM then we're in luck. The
// 16bit addresses will have an immediate part and possibly a register offset.
// The register offset is only 8bits, so it'll usually keep us in the same
// page, unless the immediate part is within the top 255 locations. Then
// we'll know if it's in page X or (X + 1), and since I/O is usually static
// that also means that we'll know at compile time which pages aren't I/O.
// When we know the page and it's static then the address mode will be simple.

// Otherwise it's direct; if the page is known but not-static then it's:
// operand = *(page_table_offset)
// operand = imm_offset[operand + reg]

// And if it can cross page boundaries then:

// operand = *page_table_offset_a
// if(reg > reg_limit)
//   operand = *page_table_offset_b;
// operand = imm_offset[operand + reg]

// (note that this can be done as a cmp + cmovgt)

// I/O is a special case, that has to be handled differently. There's a case
// where the last page can be wrapped around to I/O as well, which has to be
// specially considered.

// A lot more is involved, but this case will hopefully not be extremely
// common.

// Zero page modes are a special case of direct + static (if RAM is static).
// Same with stack.

// Indirect modes are like a combination between the page boundary crossing
// version and the zero page version, except they can also access I/O. I
// think in practice they really don't, though.

/** Code fetch operations ****************************************************/

// These are all compile time operations for the recompiler.
...
// There's now a compile time part and a run time part in the effective
// address generation. The fetch part is compile time, but the
