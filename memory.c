...
void get_bram_path(char *path)
{
  ...
  {
    sprintf(path, "%s%cbram%c%s.sav.tns", config.main_path,
     DIR_SEPARATOR_CHAR, DIR_SEPARATOR_CHAR, config.rom_filename);
  }
  else
  {
    sprintf(path, "%s%cbram%cbram.sav.tns", config.main_path,
...
s32 load_rom(char *path)
{
  ...
  if(dot_ptr != NULL)
  {
    ...
    if(!strcasecmp(dot_ptr + 1, "bin.tns"))
    {
      if((strlen(path) > 7) && (strcasecmp(dot_ptr - 7, "syscard.bin.tns")))
      {
        printf("Load cue, not bin, stupid.\n");
        strcpy(dot_ptr + 1, "cue.tns");
      }
    }

    if(!strcasecmp(dot_ptr + 1, "cue.tns"))
    {
    ...
    if(!strcasecmp(dot_ptr + 1, "sgx.tns"))
      config.sgx_mode = 1;

    if(!strcasecmp(dot_ptr + 1, "bz2.tns"))
    {
      if((strlen(path) > 4) && (!strcasecmp(dot_ptr - 4, ".sgx.bz2.tns")))
        config.sgx_mode = 1;
  ...
  sprintf(path_name, "%s.cfg.tns", config.rom_filename);
